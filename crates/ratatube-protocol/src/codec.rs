...
mod tests {
    use super::*;
    use crate::{ClientFrame, Command, DaemonFrame, ReplyBody, ReplyResult, WireEvent};
    ...
    async fn client_frames_round_trip() {
        ...
        let command = round_trip(&ClientFrame::Command {
            ...
            command: Box::new(Command::QueueAdd {
                track: ratatube_domain::media::Track::new("id", "title", "channel"),
                next: true,
            }),
        })
        .await;
    ...
    async fn snapshot_round_trips_from_domain_state() {
        let mut domain = ratatube_domain::state::DomainState::default();
        domain
            .queue
            .push(ratatube_domain::media::Track::new("a", "Title", "Channel"));
        domain.mpv_ready = true;
        let snapshot = crate::Snapshot::from(&domain);
