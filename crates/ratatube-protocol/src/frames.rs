...
use ratatube_domain::media::{Track, TrackDetails};
use ratatube_domain::playback::PlaybackSnapshot;
use ratatube_domain::playlists::Playlist;
use ratatube_domain::queue::Queue;
use ratatube_domain::state::DomainState;
...
impl WireChannel {
    ...
    pub fn from_state(state: &ratatube_domain::state::ChannelState) -> Self {
        Self {
    ...
    pub fn into_state(
        self,
        return_to: ratatube_domain::state::ChannelNavigationSnapshot,
    ) -> ratatube_domain::state::ChannelState {
        ratatube_domain::state::ChannelState {
            name: self.name,
            url: self.url,
            tracks: self.tracks,
...
pub enum WireImport {
    ...
    Review {
        summary: ratatube_domain::playlists::import::ImportSummary,
...
impl WireImport {
    ...
    pub fn from_state(state: &ratatube_domain::state::ImportState) -> Self {
        use ratatube_domain::state::ImportState;
        match state {
    ...
    pub fn into_state(self) -> ratatube_domain::state::ImportState {
        use ratatube_domain::state::ImportState;
        match self {
            Self::Fetching { url } => ImportState::Fetching {
                operation_id: ratatube_domain::operations::OperationId::mirror_placeholder(),
                url,
            },
