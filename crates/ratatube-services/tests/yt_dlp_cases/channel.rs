use ratatube_services::media::channel::{CHANNEL_PAGE_SIZE, ChannelPageRequest};
...
async fn unsafe_channel_url_is_rejected_before_process_spawn() {
    let client = ratatube_services::media::yt_dlp::YtDlp::new("/nonexistent/yt-dlp");
    let error = client
        .fetch_channel_page(&ChannelPageRequest {
    ...
    assert!(matches!(
        error,
        ratatube_services::error::AppError::InvalidUrl(_)
    ));
}
