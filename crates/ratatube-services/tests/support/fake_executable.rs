...
//! trip over.
//!
//! `#[path]`-included from the clipboard and child unit tests as well as the
//! `yt_dlp` integration test: a lib `#[cfg(test)]` module is invisible to an
//! integration test binary, and declaring a shared module in a production file
//! is not an option, so one file is included from each test-only site instead
//! of being pasted three times.
