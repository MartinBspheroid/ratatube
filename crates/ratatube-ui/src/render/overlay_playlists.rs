...
use crate::render::{centered_rect, icons, theme};
use crate::state::{AppState, PlaylistEditorField, PromptPurpose};
...
fn render_editor(
    frame: &mut ratatui::Frame,
    area: ratatui::layout::Rect,
    editor: &crate::state::PlaylistEditorState,
    theme: &theme::Theme,
) {
...
fn render_picker(
    ...
    let (create_new, matching) =
        crate::filter::picker_candidates(&state.domain.playlists, &picker.filter);
