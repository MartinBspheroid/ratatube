...
fn render_track_info(
    ...
    let mut lines = vec![
        Line::from(Span::styled(
            crate::render::widgets::truncate_middle(&sanitize_terminal_text(&track.title), width),
            theme.header,
        )),
        Line::from(Span::styled(
            crate::render::widgets::truncate_end(&sanitize_terminal_text(&track.artist), width),
            theme.accent,
        )),
        Line::from(""),
...
fn stats_line(
    ...
    if let Some(views) = details.view_count {
        values.push(format!(
            "{} views",
            ratatube_domain::media::format_count(views)
        ));
    }
    if let Some(likes) = details.like_count {
        values.push(format!(
            "{} likes",
            ratatube_domain::media::format_count(likes)
        ));
    }
...
fn detail_chips(details: Option<&ratatube_domain::media::TrackDetails>) -> Vec<String> {
    let Some(details) = details else {
        return Vec::new();
    };
    let mut values = Vec::new();
...
fn render_metadata(
    ...
    if let Some(published) =
        details.and_then(ratatube_domain::media::TrackDetails::formatted_upload_date)
    {
