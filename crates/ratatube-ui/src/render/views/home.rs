...
use crate::render::views::home_panels::{render_playlists, render_recent};
use crate::render::views::home_resume::render_resume;
use crate::state::HomeSection;

struct DashboardData<'a> {
    history: Option<&'a HistoryLog>,
    recent: &'a [ratatube_domain::media::Track],
    icons: &'a Icons,
    theme: &'a Theme,
    show_resume: bool,
}
...
fn actionable_resume(state: &AppState) -> bool {
    ...
    if state.domain.playback.status == ratatube_domain::playback::PlaybackStatus::Playing {
        return false;
    }
    state
        .domain
