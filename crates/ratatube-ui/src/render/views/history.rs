...
pub(super) fn render_history(
    ...
    let entries = history.map(HistoryLog::entries).unwrap_or(&[]);
    let recent_indices = history
        .map(HistoryLog::recent_unique_indices)
        .unwrap_or_default();
    let top = (state.ui.history_view_mode == HistoryViewMode::Top)
        .then(|| history.map(HistoryLog::aggregate).unwrap_or_default());
...
fn outcome_label(outcome: ratatube_domain::history::model::PlaybackOutcome) -> &'static str {
    match outcome {
        ratatube_domain::history::model::PlaybackOutcome::Completed => "finished",
        ratatube_domain::history::model::PlaybackOutcome::Skipped => "skipped",
        ratatube_domain::history::model::PlaybackOutcome::Failed => "failed",
        ratatube_domain::history::model::PlaybackOutcome::Stopped => "stopped",
    }
}
...
fn recent_rows(
    state: &AppState,
    entries: &[ratatube_domain::history::model::HistoryEntry],
...
fn top_rows(
    state: &AppState,
    stats: &[ratatube_domain::history::TrackStats],
    width: u16,
    icons: &Icons,
    theme: &Theme,
) -> Vec<ratatui::widgets::Row<'static>> {
