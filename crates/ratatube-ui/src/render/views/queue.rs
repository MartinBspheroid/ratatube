...
fn queue_modes(state: &AppState) -> String {
    ...
    match state.domain.queue.repeat {
        ratatube_domain::queue::RepeatMode::Track => modes.push("repeat track"),
        ratatube_domain::queue::RepeatMode::Queue => modes.push("repeat queue"),
        ratatube_domain::queue::RepeatMode::Off => {}
    }
    if modes.is_empty() {
        String::new()
    } else {
