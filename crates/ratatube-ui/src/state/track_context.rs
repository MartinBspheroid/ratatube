...
use crate::state::{AppState, HistoryViewMode, HomeSection, View};
...
pub fn resolve_track_context(
    ...
    let (track, source) = match state.ui.view {
        ...
        View::NowPlaying
            if state.ui.playing_pane == crate::state::PlayingPane::Queue
                && crate::render::layout::Breakpoint::from_width(state.ui.screen_area.width)
...
pub fn open_track_context(state: &mut AppState, history: Option<&HistoryLog>) {
    match resolve_track_context(state, history) {
        Some(context) => {
            ...
            state.ui.track_context_menu = Some(crate::state::TrackContextMenuState {
                context,
                selected: 0,
            });
        }
