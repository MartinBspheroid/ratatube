    ...
    fn playback_status_from(&mut self, event: &PlaybackEvent) {
        ...
        match event {
            PlaybackEvent::Started => {
                self.domain.playback.status = PlaybackStatus::Playing;
            }
            ...
            PlaybackEvent::PauseChanged(paused) => {
                self.domain.playback.status = if *paused {
            ...
            PlaybackEvent::VolumeChanged(v) => {
                self.domain.playback.volume = (*v).clamp(0.0, 100.0) as u8;
            }
            PlaybackEvent::MuteChanged(m) => self.domain.playback.muted = *m,
            PlaybackEvent::SpeedChanged(s) => self.domain.playback.speed = *s,
            PlaybackEvent::EndFile { .. } => self.domain.playback.status = PlaybackStatus::Stopped,
            PlaybackEvent::PlaybackError(_) | PlaybackEvent::Shutdown => {
                self.domain.playback.status = PlaybackStatus::Idle;
            }
            PlaybackEvent::Connected => self.domain.mpv_ready = true,
        }
    }
}
