...
pub(super) fn reduce(state: &mut AppState, action: PlaybackAction) -> Vec<Effect> {
    match Action::Playback(action) {
        Action::Playback(PlaybackAction::PlayPause) => {
            ...
            if let Some(pending) = &mut state.domain.pending_resume
                && !pending.armed
            {
                pending.play_on_load = true;
                return Vec::new();
            }
            state.domain.pending_resume = None;
        ...
        Action::Playback(PlaybackAction::CycleSleepTimer) => {
            ...
            let minutes = match state.domain.sleep_timer.map(|t| t.minutes) {
                None => Some(15),
                Some(15) => Some(30),
                Some(30) => Some(60),
                Some(_) => None,
            };
            state.domain.sleep_timer = minutes.map(|m| SleepTimer {
        ...
        Action::Playback(PlaybackAction::ToggleRadio) => {
            state.domain.radio = !state.domain.radio;
            if !state.domain.radio {
