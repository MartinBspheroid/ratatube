...
pub(super) fn reduce(state: &mut AppState, action: PlaylistAction) -> Vec<Effect> {
    match Action::Playlists(action) {
        Action::Playlists(PlaylistAction::PickerInput(c)) => {
            if let Some(picker) = &mut state.ui.picker {
        ...
        Action::Playlists(PlaylistAction::PickerCancel) => state.ui.picker = None,
        Action::Playlists(PlaylistAction::OpenPrompt(purpose)) => {
            state.ui.prompt = Some(crate::app::state::PromptState {
                purpose,
                buffer: String::new(),
            });
        }
        Action::Playlists(PlaylistAction::PromptInput(c)) => {
            if let Some(prompt) = &mut state.ui.prompt
        ...
        Action::Playlists(PlaylistAction::PromptCancel) => state.ui.prompt = None,
        Action::Playlists(PlaylistAction::OpenPlaylistEditor) => {
            state.ui.playlist_editor = state
                .ui
                .selected_playlist
        ...
        Action::Playlists(PlaylistAction::PlaylistEditorInput(character)) => {
            if let Some(editor) = &mut state.ui.playlist_editor {
                match editor.field {
        ...
        Action::Playlists(PlaylistAction::PlaylistEditorCancel) => state.ui.playlist_editor = None,
        Action::Playlists(PlaylistAction::ConfirmNo) => state.ui.confirm = None,
...
fn picker_move(state: &mut AppState, delta: i32) {
    let Some(picker) = &state.ui.picker else {
        return;
    };
    ...
    if total > 0
        && let Some(picker) = &mut state.ui.picker
    {
