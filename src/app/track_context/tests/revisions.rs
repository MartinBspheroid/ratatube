...
fn queue_revision_advances_for_play_add_and_shuffle_order_changes() {
    ...
    let initial = state.queue_revision;

    reducer::reduce(
        &mut state,
        Action::Playback(PlaybackAction::PlayTrack(track("one", "One"))),
    );
    assert_ne!(state.queue_revision, initial);
    let after_add = state.queue_revision;

    reducer::reduce(&mut state, Action::Playback(PlaybackAction::ToggleShuffle));
    assert_ne!(state.queue_revision, after_add);
}
...
async fn playlist_and_queue_load_mutations_advance_collection_revisions() {
    ...
    let playlist_revision = app.state.playlists_revision;
    reducer::reduce(
        &mut app.state,
        Action::Playlists(PlaylistAction::PlaylistSaved(playlist)),
    );
    assert_ne!(app.state.playlists_revision, playlist_revision);

    let queue_revision = app.state.queue_revision;
    ...
    .await;
    assert_ne!(app.state.queue_revision, queue_revision);
}
