...
fn home_is_the_landing_view() {
    ...
    assert!(
        out.contains("Welcome to ratatube"),
        "first-run hero:\n{out}"
    );
    assert!(
...
fn home_shows_armed_resume_card() {
    ...
    state.domain.pending_resume = Some(ratatube::app::state::PendingResume {
        track,
...
fn home_hides_quick_resume_after_playback_has_moved_on() {
    ...
    for (current, status) in [
        ...
        state
            .domain
            .playlists
            .push(ratatube::playlists::Playlist::new("Kept dashboard"));
...
fn home_dashboard_exercises_all_four_breakpoints() {
    ...
    let render = |width: u16, height: u16| {
        ...
        let mut playlist = ratatube::playlists::Playlist::new("Playlist width-safe");
        playlist
            .tracks
            .push(ratatube::playlists::model::PlaylistTrack::from(&recent));
...
fn home_playlist_tiles_page_with_semantic_chevrons() {
    let mut state = AppState::new();
    state.ui.home_section = ratatube::app::state::HomeSection::Playlists;
    state.ui.selected_index = 4;
    state.domain.playlists = (1..=5)
        .map(|index| ratatube::playlists::Playlist::new(format!("Paged playlist {index}")))
        .collect();
...
fn home_omits_activity_and_continue_listening_data() {
    use ratatube::history::activity::{ActivityEvent, ActivityKind};
